...
export async function updateAiCurrentContext(
  ...
  await mutateLocalData([db.aiMemorySettings], async () => {
...
export async function createAiMemorySummary(args: {
  ...
  const now = Date.now();
  const id = crypto.randomUUID();
  await mutateLocalData([db.aiMemorySummaries], async () => {
...
export async function advanceFourMonthEpoch(
  nextStartAt: number,
): Promise<void> {
  await mutateLocalData([db.aiMemorySettings], async () => {
    await db.aiMemorySettings.update(AI_MEMORY_SETTINGS_ID, {
...
export async function mergeCodexCloudMemory(args: {
  ...
  // Cloud-owned cache write. Deliberately does NOT advance the local revision:
  // mirroring what the cloud already holds back to the cloud would be a sync
  // loop, and this data did not originate on this device.
  const now = Date.now();
